# Timeline Plan / Done PWA v11 — iPhone / iPad 可安装版
...
- 增加 iPhone / iPad 安装按钮与安装说明弹窗。
- 增加 iOS 专用 `apple-touch-icon` 图标。
- 增加 `mobile-web-app-capable`、`apple-mobile-web-app-capable`、`apple-mobile-web-app-title` 等主屏幕 Web App meta 标签。
- Service Worker 缓存更新为 v11，并缓存 iOS 主屏幕图标。
- v10 数据会自动迁移到 v11。
...
把整个 `timeline-plan-done-pwa-v11` 文件夹部署到任一 HTTPS 静态托管，例如：
